...
Run a live-database verification script with `npx tsx scripts/<script>.ts`. The server intentionally has no unit-test runner; `npm test` exits with the package's placeholder failure.
...
- Import the generated Prisma client from the relative path `../../generated/prisma/client`; do not import `@prisma/client` directly. Run `npm run generate` after schema changes.
...
- The Postman collection is the API contract. Update `postman/Ecom.postman_collection.json` with API changes and run `npm run verify:postman`; keep the admin/frontend copies synchronized when the change affects their contract.
