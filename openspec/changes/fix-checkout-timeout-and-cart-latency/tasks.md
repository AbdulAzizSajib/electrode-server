...
- [x] 3.4 Leave `deductStockForOrderItem` and the transaction structure unchanged — explicitly out of scope per design D2 item 4 — verified untouched
- [ ] 3.5 Measure a checkout end-to-end before and after 3.1–3.3 and record both numbers in the change notes; this is what tells you whether 30s in task 4.2 is comfortable or merely adequate — **needs a running server + database; left for you.** Expected saving: (N-1) stock aggregates + up to 5 order-number reads + N low-stock notification chains, all removed from the request path
